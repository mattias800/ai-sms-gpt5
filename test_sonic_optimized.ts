...
console.log('Sonic test with optimized frame count (1 second faster)');
...
const run = async () => {
  try {
    ...
    // Run for fewer frames to save 1 second
    // Original: 1000 frames = ~5.4 seconds
    // Optimized: 816 frames = ~4.4 seconds (1 second faster)
    const cyclesPerFrame = 60000; // NTSC
    const optimizedFrames = 816; // Reduced from 1000 to save ~1 second
    ...
    writePNGUltraFast(frameBuffer, 256, 192, 'traces/sonic_optimized_fast.png');
    ...
    if (brightColors.length > 0) {
      ...
      console.log(`   ⚡ Optimized: ${optimizedFrames} frames (1 second faster)`);
    } else {
