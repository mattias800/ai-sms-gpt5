...
describe('Z80 disassembler basics', (): void => {
  it('disassembles base opcodes (IN/OUT/LD/JP/JR/DJNZ)', (): void => {
  ...
  it('disassembles ED IM and LD (nn),ss / ss,(nn)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    const seq = [
      0xed, 0x5e,       // IM 2
  ...
  it('disassembles DD/FD indexed forms', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    const seq = [
