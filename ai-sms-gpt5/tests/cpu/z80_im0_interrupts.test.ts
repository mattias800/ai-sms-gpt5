...
describe('Z80 IM0 interrupts', (): void => {
  it('default IM0 behaves like RST 38h (vector 0x0038), 13 cycles, pushes return address and clears IFF1', (): void => {
  ...
  it('IM0 configurable vector jumps to provided address (e.g., 0x0028)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: IM 0; EI; NOP; HALT
    mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
  ...
  it('IM0 executes injected RST opcode (RST 20h via 0xE7)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: IM 0; EI; NOP; HALT
    mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
  ...
  it('IM0 throws for unsupported injected opcode (e.g., NOP 0x00)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: IM 0; EI; NOP; HALT
    mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
  ...
  it('setIM0Opcode(null) returns to vector mode', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: IM 0; EI; NOP; HALT
    mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
