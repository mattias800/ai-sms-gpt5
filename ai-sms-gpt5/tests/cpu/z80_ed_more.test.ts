...
describe('Z80 ED-prefixed more cases', (): void => {
  it('LD (nn),SP then LD SP,(nn) roundtrip (cycles 20 each)', (): void => {
  ...
  it('RETN and RETI pop PC and set IFF1 = IFF2', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: RETN; HALT
  ...
  it('IM 0 and IM 2 set mode and take 8 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('ADC HL,SP and SBC HL,HL results and flags basics', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
