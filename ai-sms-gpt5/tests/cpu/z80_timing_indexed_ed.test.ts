...
describe('Z80 timing: ED and DD/FD indexed instructions', (): void => {
  it('ED ADC HL,ss all variants are 15 cycles', (): void => {
    ...
    c = step(cpu); expect(c).toBe(15);
    c = step(cpu); expect(c).toBe(15);
    c = step(cpu); expect(c).toBe(15);
  });

  it('ED SBC HL,ss all variants are 15 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('DD/FD ADD IX/IY,pp are 15 cycles', (): void => {
  ...
  it('JP (IX/IY) take 8 cycles', (): void => {
    // JP (IX)
    let bus = new SimpleBus();
    let mem = bus.getMemory();
  ...
  it('LD (IX+d),n and LD A,(IX+d) and LD (IX+d),A take 19 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: DD 36 01 77 ; DD 7E 02 ; DD 77 03
    mem.set([0xdd, 0x36, 0x01, 0x77, 0xdd, 0x7e, 0x02, 0xdd, 0x77, 0x03], 0x0000);
    const cpu = createZ80({ bus });
    let c = step(cpu); expect(c).toBe(19); // LD (IX+1),0x77
    c = step(cpu); expect(c).toBe(19); // LD A,(IX+2)
    c = step(cpu); expect(c).toBe(19); // LD (IX+3),A
  });

  it('ALU (IX+d) forms (ADC/CP) are 19 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // ADC A,(IX+0) ; CP (IX+5)
  ...
  it('LD r,(IX+d) and LD (IX+d),r are each 19 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // LD B,(IX+0) ; LD (IX+1),C
    ...
    let c = step(cpu); expect(c).toBe(19); // LD B,(IX+0)
    c = step(cpu); expect(c).toBe(19);     // LD (IX+1),C
  });

  it('EX (SP),IX/IY are 23 cycles', (): void => {
    // EX (SP),IX
    let bus = new SimpleBus();
    let mem = bus.getMemory();
    ...
    let c = step(cpu); expect(c).toBe(23);
