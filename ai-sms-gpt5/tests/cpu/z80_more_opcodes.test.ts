...
describe('Z80 additional opcodes: 16-bit ops, EX/stack, LD (nn),A/A,(nn), IO, IX/IY basics', (): void => {
  it('LD dd,nn and INC/DEC dd and ADD HL,ss', (): void => {
    ...
    let c = step(cpu); expect(c).toBe(10);
    c = step(cpu); expect(c).toBe(10);
    c = step(cpu); expect(c).toBe(10);
    c = step(cpu); expect(c).toBe(10);
    c = step(cpu); expect(c).toBe(11);
  ...
  it('LD (nn),A and LD A,(nn); LD (BC/DE),A and A,(BC/DE)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // LD A,0x5A; LD (4002),A; LD A,0; LD A,(4002)
    mem.set([0x3e, 0x5a, 0x32, 0x02, 0x40, 0x3e, 0x00, 0x3a, 0x02, 0x40], 0x0000);
    const cpu = createZ80({ bus });
    let c = step(cpu); expect(c).toBe(7);
    c = step(cpu); expect(c).toBe(13);
    expect(mem[0x4002]).toBe(0x5a);
    c = step(cpu); expect(c).toBe(7);
    c = step(cpu); expect(c).toBe(13);
    ...
    c = step(cpu); expect(c).toBe(7); // (BC),A
    expect(mem[0x5000]).toBe(0xaa);
    c = step(cpu); expect(c).toBe(7);
    expect(cpu.getState().a).toBe(0xaa);
  });

  it('PUSH/POP AF and EX DE,HL and EX (SP),HL', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    ...
    const st = cpu.getState();
    expect(st.h).toBe(0x12); expect(st.l).toBe(0x34);
    expect(mem[0x9000]).toBe(0x78); expect(mem[0x9001]).toBe(0x56);
  });

  it('IN A,(n) sets A and flags; OUT (n),A writes', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    mem.set([0xdb, 0x7f, 0xd3, 0x7f], 0x0000);
    const cpu = createZ80({ bus });
    let c = step(cpu); expect(c).toBe(11);
    const a1 = cpu.getState().a; // SimpleBus returns 0xFF
    expect(a1).toBe(0xff);
    c = step(cpu); expect(c).toBe(11);
  });

  it('DD: LD IX,nn and LD SP,IX and EX (SP),IX and ADC A,(IX+d)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    ...
    const cpu = createZ80({ bus });
    // LD IX,9000
    let c = step(cpu); expect(c).toBe(14);
    // LD SP,IX
    c = step(cpu); expect(c).toBe(10);
    // LD IX,0x1234
