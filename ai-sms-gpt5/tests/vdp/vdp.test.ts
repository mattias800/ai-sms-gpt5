...
describe('VDP ports and timing', (): void => {
  it('status (0xBF) read returns and clears VBlank/IRQ state', (): void => {
    const vdp = createVDP();
  ...
  it('enabling IRQ during active VBlank immediately asserts the line', (): void => {
    const vdp = createVDP();
  ...
  it('frame wraps after linesPerFrame and continues ticking', (): void => {
    const vdp = createVDP();
  ...
  it('reg1 bit5 disable clears IRQ line; reg15 sets autoincrement used by data port', (): void => {
    const vdp = createVDP();
  ...
  it('reg15=0 is guarded to 1 autoincrement in this stub (avoid 0)', (): void => {
    const vdp = createVDP();
