...
describe('bit utils', (): void => {
  ...
  it('property: u8 and u16 idempotent and consistent', (): void => {
    fc.assert(
      fc.property(fc.integer({ min: -1e9, max: 1e9 }), (n: number): void => {
        const a = u8(n);
        const b = u8(a);
        expect(a).toBe(b);
        const c = u16(n);
