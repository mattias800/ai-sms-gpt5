...
(async () => {
  ...
  if (!existsSync(biosPath)) { console.error('BIOS missing'); process.exit(1); }
  const bios = new Uint8Array(readFileSync(biosPath));

  const m = createMachine({ cart: { rom: new Uint8Array(0) }, useManualInit: false, bus: { bios } });
  ...
  for (let f = 1; f <= 200; f++) {
    ...
    if (white > 0 && darkBlue > 0 && other === 0) {
      firstLogoFrame = f;
      capture = { f, modalHSV, white, darkBlue, other, bg };
      break;
    }
  }

  console.log(JSON.stringify({ firstLogoFrame, capture }, null, 2));
})();
