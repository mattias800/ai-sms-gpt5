...
function renderFrame(
  ...
  for (let ty = 0; ty < 28; ty++) {
    ...
    for (let tx = 0; tx < TILES_X; tx++) {
      ...
      // SMS Mode 4: tile index is 9 bits (bit 0 of high byte + 8 bits of low byte)
      const tileIndex = ((high & 0x01) << 8) | low;
...
console.log('\nRunning for 420 frames to let graphics load...');
...
const rgb = renderFrame(
  vdpState?.vram ?? new Uint8Array(0x4000),
  vdpState?.cram ?? new Array(32).fill(0),
  nameBase,
  patternBase,
  false
);
