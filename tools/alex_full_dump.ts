...
const nameTableBase = ((vdpState.regs?.[2] ?? 0 ?? 0) & 0x0e) << 10;
...
const patternBaseBit = (vdpState.regs?.[4] ?? 0 ?? 0) & 0x04 ? 0x2000 : 0x0000;
...
for (const tile of tilesToCheck) {
  ...
  if (!hasData) {
    ...
    const b0 = vdpState.vram[addr] ?? 0 ?? 0;
    const b1 = vdpState.vram[addr + 1] ?? 0 ?? 0;
    const b2 = vdpState.vram[addr + 2] ?? 0 ?? 0;
    const b3 = vdpState.vram[addr + 3] ?? 0 ?? 0;
    let pixels = '';
    for (let col = 0; col < 8; col++) {
      const bit = 7 - col;
      const color = ((b0 >> bit) & 1) | (((b1 >> bit) & 1) << 1) | (((b2 >> bit) & 1) << 2) | (((b3 >> bit) & 1) << 3);
