...
if (vdpState) {
  ...
  console.log(
    `  R1: Mode control 2 - Display:${(vdpState.regs?.[1] ?? 0 ?? 0) & 0x40 ? 'ON' : 'OFF'}, VBlank IRQ:${(vdpState.regs?.[1] ?? 0 ?? 0) & 0x20 ? 'ON' : 'OFF'}`
  );
  console.log(`  R2: Name table base = 0x${(((vdpState.regs?.[2] ?? 0 ?? 0) & 0x0e) << 10).toString(16)}`);
  console.log(`  R3: Color table base (unused in SMS mode)`);
  console.log(`  R4: Pattern table base = 0x${(((vdpState.regs?.[4] ?? 0 ?? 0) & 0x07) << 11).toString(16)}`);
  console.log(`  R5: Sprite attribute table base = 0x${(((vdpState.regs?.[5] ?? 0 ?? 0) & 0x7e) << 7).toString(16)}`);
  console.log(`  R6: Sprite pattern table base = 0x${(((vdpState.regs?.[6] ?? 0 ?? 0) & 0x07) << 11).toString(16)}`);
  console.log(`  R7: Border color = ${(vdpState.regs?.[7] ?? 0 ?? 0) & 0x0f}`);
  console.log(`  R8: Horizontal scroll = ${vdpState.regs?.[8] ?? 0 ?? 0}`);
  console.log(`  R9: Vertical scroll = ${vdpState.regs?.[9] ?? 0 ?? 0}`);
  console.log(`  R10: Line interrupt counter = ${vdpState.regs?.[10] ?? 0 ?? 0}`);
  ...
  for (let i = 0; i < vdpState.vram.length && count < 10; i++) {
    if ((vdpState.vram[i] ?? 0) !== 0) {
      console.log(
        `  VRAM[0x${i.toString(16).padStart(4, '0')}] = 0x${(vdpState.vram[i] ?? 0)!.toString(16).padStart(2, '0')}`
      );
      count++;
    }
  }
...
for (let frame = 0; frame < 120; frame++) {
  ...
  if (frame % 60 === 0) {
    ...
    if (vdpSt) {
      console.log(
        `Frame ${60 + frame}: Display=${vdpSt.displayEnabled}, PC=0x${cpuSt.pc.toString(16).padStart(4, '0')}, VRAM writes=${vdpSt.vramWrites}`
      );
    }
  }
}
