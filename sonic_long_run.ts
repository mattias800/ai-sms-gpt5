...
console.log('Running Sonic for extended frames to find title screen');
...
// Test frames from 1000 to 2000 in steps of 100
const testFrames = [];
...
for (const targetFrame of testFrames) {
  ...
  try {
    ...
    const filename = `traces/sonic_frame_${targetFrame}_long.png`;
    ...
    // Early exit if we find a good title screen
    if (score > 100) {
      console.log(`  🎉 EXCELLENT TITLE SCREEN FOUND!`);
      break;
    }
...
if (bestScore > 100) {
  ...
  console.log(`\n📸 Check: traces/sonic_frame_${bestFrame}_long.png`);
} else if (bestScore > 50) {
  console.log(`✅ GOOD: Frame ${bestFrame} is acceptable`);
} else {
  console.log(`⚠️ POOR: No good title screen found in frames 1000-2000`);
  console.log(`   Sonic might need even longer to reach title screen`);
  console.log(`   Or there might be an issue with the ROM or emulation`);
}
