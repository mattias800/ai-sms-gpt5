...
const main = (): number => {
  console.log('Wonder Boy with automatic background color fix\n');
  ...
  if (!isLightBlueish(bg[0],bg[1],bg[2])) { 
    ...
    dumpFrame(frame,'wonderboy_fixed_fail.png'); 
    return 1; 
  }
  if (bgShare < 0.60) { 
  ...
  // Check center 10x10 area for SEGA logo
  ...
  for (let y=startY;y<startY+10;y++) for (let x=startX;x<startX+10;x++) { 
    const [r,g,b]=rgbAt(frame,x,y); 
    const k=keyOf(r,g,b); 
    if (k===bgKey) continue; 
    nonBgCenter++; 
  ...
  if (nonBgCenter===0) { 
    console.error('FAIL: Center 10x10 contains only background'); 
    dumpFrame(frame,'wonderboy_fixed_fail.png'); 
    return 1; 
  }
  if (unexpected.size>0) { 
    console.error(`FAIL: Center 10x10 has unexpected colors: ${Array.from(unexpected).join(' | ')}`); 
    dumpFrame(frame,'wonderboy_fixed_fail.png'); 
    return 1; 
  }
  if (sawWhite===0 || sawDarkBlue===0) { 
    console.error(`FAIL: Center lacks both white and darker blue (white=${sawWhite}, darkBlue=${sawDarkBlue})`); 
    dumpFrame(frame,'wonderboy_fixed_fail.png'); 
    return 1; 
  }

  console.log('✅ PASS: Background light blue; center contains white + darker blue (SEGA logo).');
  dumpFrame(frame,'wonderboy_fixed.png');
  return 0;
};
