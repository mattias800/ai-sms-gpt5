...
console.log('Comprehensive accuracy analysis of Spy vs Spy title screen');

// Load our latest screenshot
...
if (bluePercentage > 70) {
  score += 25;
  console.log(`✅ Background color: Excellent (${bluePercentage}% blue)`);
} else if (bluePercentage > 50) {
  score += 20;
  console.log(`✅ Background color: Good (${bluePercentage}% blue)`);
} else if (bluePercentage > 30) {
  score += 15;
  console.log(`⚠️ Background color: Acceptable (${bluePercentage}% blue)`);
} else {
  console.log(`❌ Background color: Poor (${bluePercentage}% blue)`);
}

// Corner consistency check
maxScore += 25;
const cornerBlueCount = cornerRegions.filter(corner => {
  const colors = new Map<string, number>();
  for (let dy = 0; dy < corner.size; dy++) {
    for (let dx = 0; dx < corner.size; dx++) {
      const x = corner.x + dx;
  ...
  if (dominantColor) {
    ...
    const [r, g, b] = colorKey.split(',').map(Number);
    return r === 0 && g === 85 && b === 255;
  }
  return false;
}).length;

if (cornerBlueCount === 4) {
  score += 25;
  console.log(`✅ Corner consistency: Perfect (all 4 corners blue)`);
} else if (cornerBlueCount >= 3) {
  score += 20;
  console.log(`✅ Corner consistency: Good (${cornerBlueCount}/4 corners blue)`);
} else if (cornerBlueCount >= 2) {
  score += 15;
  console.log(`⚠️ Corner consistency: Acceptable (${cornerBlueCount}/4 corners blue)`);
} else {
  console.log(`❌ Corner consistency: Poor (${cornerBlueCount}/4 corners blue)`);
}
...
console.log(`\n🎯 Overall Accuracy Score: ${score}/${maxScore} (${finalScore}%)`);

if (parseFloat(finalScore) >= 90) {
  console.log(`🎉 EXCELLENT: Very accurate emulation!`);
} else if (parseFloat(finalScore) >= 80) {
  console.log(`✅ GOOD: Mostly accurate with minor issues`);
} else if (parseFloat(finalScore) >= 70) {
  console.log(`⚠️ ACCEPTABLE: Some accuracy issues`);
} else {
  console.log(`❌ POOR: Significant accuracy issues`);
}
...
if (bluePercentage < 70) {
  console.log(`⚠️ Background not blue enough: ${bluePercentage}% (should be >70%)`);
}

if (cornerBlueCount < 4) {
  console.log(`⚠️ Corner inconsistency: ${cornerBlueCount}/4 corners are blue`);
}

if (colorMap.size < 8) {
  console.log(`⚠️ Low color diversity: ${colorMap.size} colors (should be >8)`);
}

if (centerWhitePercentage < 10) {
  console.log(`⚠️ Text readability: ${centerWhitePercentage}% white in center (should be >10%)`);
}
...
const unexpectedColors = Array.from(colorDetails.entries())
  .filter(([colorKey, details]) => {
    const [r, g, b] = colorKey.split(',').map(Number);
    // Check for colors that shouldn't be in Spy vs Spy
    return (
...
console.log(`\n=== Summary ===`);
console.log(`Our Spy vs Spy emulation shows:`);
console.log(`- Blue background: ${bluePercentage}%`);
console.log(`- Corner consistency: ${cornerBlueCount}/4 blue`);
console.log(`- Color diversity: ${colorMap.size} colors`);
console.log(`- Text readability: ${centerWhitePercentage}% white in center`);
console.log(`- Overall accuracy: ${finalScore}%`);

if (parseFloat(finalScore) >= 90) {
  console.log(`\n🎉 This is very close to perfect! The remaining differences might be:`);
  console.log(`1. Minor timing differences`);
  console.log(`2. Subtle color variations`);
  console.log(`3. Small positioning differences`);
  console.log(`4. Font rendering differences`);
} else {
  console.log(`\n🔍 Areas for improvement:`);
  if (bluePercentage < 70) console.log(`- Increase blue background coverage`);
  if (cornerBlueCount < 4) console.log(`- Fix corner color consistency`);
  if (colorMap.size < 8) console.log(`- Improve color diversity`);
  if (centerWhitePercentage < 10) console.log(`- Enhance text readability`);
}
