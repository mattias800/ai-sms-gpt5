...
const main = (): number => {
  console.log('Wonder Boy with corrected SEGA logo position test\n');
  ...
  if (!isLightBlueish(bg[0],bg[1],bg[2])) { 
    ...
    dumpFrame(frame,'wonderboy_corrected_fail.png'); 
    return 1; 
  }
  if (bgShare < 0.60) { 
  ...
  if (unexpected.size > 0) { 
    console.error(`FAIL: SEGA logo area has unexpected colors: ${Array.from(unexpected).join(' | ')}`); 
    dumpFrame(frame,'wonderboy_corrected_fail.png'); 
    return 1; 
  }
  ...
  console.log('✅ PASS: Background light blue; SEGA logo area contains white + darker blue.');
  dumpFrame(frame,'wonderboy_corrected.png');
  return 0;
};
