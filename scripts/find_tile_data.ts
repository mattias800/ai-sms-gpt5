...
for (let i = 0; i < Math.min(5, sorted.length); i++) {
  ...
  if (!entry) continue;
  const [tileIndex, count] = entry;
  const addr0 = (tileIndex << 5) & 0x3fff;
  const addr2000 = (0x2000 + (tileIndex << 5)) & 0x3fff;
  const has0 = (vdpState.vram[addr0] ?? 0) !== 0 || (vdpState.vram[addr0 + 1] ?? 0) !== 0;
  const has2000 = (vdpState.vram[addr2000] ?? 0) !== 0 || (vdpState.vram[addr2000 + 1] ?? 0) !== 0;
