...
const tilesWithData: Set<number> = new Set();
for (let tile = 0; tile < 512; tile++) {
  // Check up to 512 tiles
  const addr = tile << 5;
...
const nameBase: number = ((vdpState.regs[2] ?? 0) & 0x0e) << 10;
const referencedTiles: Map<number, number> = new Map();

for (let i = 0; i < 32 * 28 * 2; i += 2) {
  // Check 28 rows
...
console.log(
  'Tiles with pattern data:',
  Array.from(tilesWithData).sort((a, b) => a - b)
);
...
console.log(
  'Referenced tiles WITH data:',
  hasDataUsed.sort((a, b) => a - b)
);
console.log(
  'Referenced tiles WITHOUT data:',
  missingData.sort((a, b) => a - b).slice(0, 20),
...
const spriteBase = ((vdpState.regs[5] ?? 0) & 0x7e) << 7;
