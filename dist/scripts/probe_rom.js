...
const findTagPositions = (rom, tag) => {
...
const main = () => {
    ...
    if (tagPos.length > 0) {
        console.log(`Found 'TMR SEGA' signature at offsets: ${tagPos.map((p) => '0x' + p.toString(16)).join(', ')}`);
    }
    else {
    ...
    const vdpRegWrites = new Array(32).fill(0);
    let vdpReg1Enabled = false;
    const vdpRegEvents = [];
    const vdpProxy = {
        readPort: (p) => vdp.readPort(p),
        writePort: (p, v) => {
            const port = p & 0xff;
            const val = v & 0xff;
            if (port === 0xbe)
    ...
    const busProxy = {
        ...
        write8: (addr, val) => bus.write8(addr, val),
        readIO8: (port) => {
            const p = port & 0xff;
            if (p === 0x7e) {
                ioReads7E++;
                if (opts.override7E !== undefined)
                    return opts.override7E & 0xff;
            }
            if (p === 0xfe) {
                ioReadsFE++;
                if (opts.overrideFE !== undefined)
                    return opts.overrideFE & 0xff;
            }
            ioReadsOther++;
            return bus.readIO8(p);
        },
        writeIO8: (port, val) => bus.writeIO8(port, val),
    };
...
main();
//# sourceMappingURL=probe_rom.js.map