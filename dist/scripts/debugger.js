...
const main = () => {
    ...
    let traceIOAllSkip = null;
    // Memory write breakpoints
    const breakMemWrite = new Set();
    const breakMemWriteRanges = [];
    const breakMemWriteVals = [];
    const busProxy = {
        read8: (addr) => bus.read8(addr),
        write8: (addr, val) => {
            const a = addr & 0xffff;
        ...
        readIO8: (port) => {
            ...
            lastIOIsWrite = false;
            if (breakIo.has(p))
                brokeOn = `io-read ${p.toString(16)}`;
            if (breakStatusRead && p === 0xbf)
                brokeOn = 'status-read';
            let ret;
            if (ioOverrides.has(p))
                ret = ioOverrides.get(p) & 0xff;
            else if ((p === 0xdc || p === 0xdd) && padOverrides.has(p))
                ret = padOverrides.get(p) & 0xff;
            else
                ret = bus.readIO8(p);
            ...
            if (traceIOAllRemain > 0 && (traceIOAllSkip === null || p !== (traceIOAllSkip & 0xff))) {
                traceReadLog.push(`pc=${hex(lastPc, 4)} port=${hex(p)} val=${hex(ret)} line=${ln} hc=${hex(hc)}`);
                traceIOAllRemain--;
            }
            if (breakIOReadMask !== null) {
                ...
                if (portOk && matchVal === wantVal)
                    brokeOn = `io-read-match p=${hex(p)} v=${hex(ret)}`;
            }
            return ret & 0xff;
        },
        writeIO8: (port, val) => {
            ...
            if (breakIo.has(p))
                brokeOn = `io-write ${p.toString(16)}`;
    ...
    const cpu = createZ80({
        ...
        waitStates: {
            ...
            ioPenalty: (port, isWrite) => {
                const p = port & 0xff;
                const low6 = p & 0x3f;
                // Model approximate wait-states for VDP and PSG I/O
                if (p === 0x7f)
                    return 4; // PSG write-only (approx)
                if (p === 0x7e || p === 0x7f)
                    return 4; // H/V counter accesses (approx)
                if (low6 === 0x3e || low6 === 0x3f)
                    return 4; // VDP data/control mirrors (approx)
                return 0;
            },
        },
    });
    ...
    const dumpMem = (addr, len) => {
        const a0 = addr & 0xffff;
        const bytes = [];
        for (let off = 0; off < len; off++)
            bytes.push(hex(bus.read8((a0 + off) & 0xffff)));
        console.log(`${hex(a0, 4)}: ${bytes.join(' ')}`);
    };
    const disasm = (addr, count) => {
        let a = addr & 0xffff;
        for (let n = 0; n < count; n++) {
            const r = disassembleOne((ad) => bus.read8(ad & 0xffff) & 0xff, a);
            const bytes = r.bytes.map((b) => hex(b)).join(' ');
    ...
    const runWithBreaks = (maxSteps, untilIrq) => {
        let cyc = 0;
        brokeOn = null;
        for (let k = 0; k < maxSteps; k++) {
            ...
            if (brokeOn)
                break; // IO break was set by proxy
        }
        if (brokeOn)
            console.log(`break: ${brokeOn}`);
        ...
        switch (cmd) {
            ...
            case 'wmem': {
                ...
                for (let k = 0; k < vals.length; k++)
                    bus.write8((a + k) & 0xffff, (vals[k] ?? 0) & 0xff);
            ...
            case 'setr': {
                ...
                const set8 = (k, val) => { st[k] = val & 0xff; };
                const set16 = (k, val) => { st[k] = val & 0xffff; };
            ...
            case 'iocmask': {
                ...
                bus.__setIOMaskForTest?.(dir & 0xff, out & 0xff);
            ...
            case 'showtrace': {
                if (traceReadLog.length === 0)
                    console.log('(no trace)');
                else
                    for (const line of traceReadLog)
                        console.log(line);
                break;
            }
            case 'cleartrace': {
            ...
            case 'iostats': {
                ...
                const entries = [];
                for (let p = 0; p < 256; p++)
                    if ((ioReadCounts[p] ?? 0) > 0)
                        entries.push({ p, c: ioReadCounts[p] });
            ...
            case 'iowstats': {
                const entries = [];
                for (let p = 0; p < 256; p++)
                    if ((ioWriteCounts[p] ?? 0) > 0)
                        entries.push({ p, c: ioWriteCounts[p] });
                entries.sort((a, b) => b.c - a.c);
                const line = entries.map((e) => `${hex(e.p)}:${e.c}`).join(' ');
            ...
            case 'pcsample': {
                ...
                if (arg === 'off')
                    ...
                    pcSampleInterval = Math.max(1, toNum(arg) | 0);
            ...
            case 'pcwatch': {
                ...
                if (sub === 'clear') {
                    ...
                    if (pcWatchSet.size === 0)
                        console.log('(none)');
                    else
                        console.log(Array.from(pcWatchSet).map((a) => hex(a, 4)).join(' '));
                }
                else if (sub === 'del') {
                    const a = toNum(cmds[i++]);
            ...
            case 'showpcwatch': {
                if (pcWatchLog.length === 0)
                    console.log('(no pcwatch hits)');
                else
                    for (const line of pcWatchLog)
                        console.log(line);
                break;
            }
            case 'clearpcwatch': {
            ...
            case 'showpcpath': {
                if (pcPathLog.length === 0)
                    console.log('(no pcpath)');
                else
                    console.log(pcPathLog.map((pc) => hex(pc, 4)).join(' -> '));
                break;
            }
            case 'clearpcpath': {
                pcPathRemain = 0;
            ...
            case 'vdp': {
                ...
                const regsStr = gs.regs.slice(0, 16).map((r, i) => `R${i}=${hex(r)}`).join(' ');
            ...
            case 'vram': {
                ...
                const bytes = [];
                for (let k = 0; k < len; k++)
                    bytes.push(hex(vdp.readPort(0xbe)));
            ...
            case 'cram': {
                ...
                const bytes = gs.cram.map((x) => hex(x)).join(' ');
            ...
            case 'renderocc': {
                ...
                const tileHasData = (tileIndex) => {
                    const addr = (patBase + ((tileIndex & 0x3ff) << 5)) & 0x3fff; // *32 bytes
                    vdp.writePort(0xbf, addr & 0xff);
                    vdp.writePort(0xbf, (addr >>> 8) & 0x3f);
                    void vdp.readPort(0xbe);
                    for (let i = 0; i < 32; i++) {
                        if ((vdp.readPort(0xbe) & 0xff) !== 0)
                            return true;
                    }
                    return false;
                };
            ...
            case 'findbytes': {
                ...
                outer: for (let a = 0; a <= 0xffff && matches.length < lim; a++) {
                    for (let k = 0; k < bytes.length; k++) {
                        if ((bus.read8((a + k) & 0xffff) & 0xff) !== bytes[k])
                            continue outer;
                    }
                    matches.push(a);
                }
