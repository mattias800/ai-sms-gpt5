...
const buildRom = () => {
    ...
    // Program at 0x0000
    let a = 0x0000;
    const emit = (b) => { rom[a++] = b & 0xff; };
    const outBF = () => { emit(0xD3); emit(0xBF); };
    ...
    a = 0x0038;
    const outBE = () => { emit(0xD3); emit(0xBE); };
...
describe('Full init flow integration (IRQ -> CRAM -> display enable)', () => {
    it('after first VBlank interrupt, handler writes CRAM and enables display', () => {
        const cart = { rom: buildRom() };
...
//# sourceMappingURL=init_flow_integration.test.js.map