...
describe('Z80 disassembler additional coverage', () => {
    it('covers ED IN/OUT (C) variants including r=6, ADC/SBC HL,ss, RRD/RLD, and fallbacks', () => {
        ...
        const seq = [
            ...
            0xed, 0x41, // OUT (C),B
            0xed, 0x79, // OUT (C),A
            0xed, 0x71, // OUT (C),0 (r=6)
            0xed, 0x4a, // ADC HL,BC
            0xed, 0x52, // SBC HL,DE
            0xed, 0x67, // RRD
            0xed, 0x6f, // RLD
            0xed, 0x00, // ED 00 fallback
        ];
        mem.set(seq, 0x0300);
        const rd = wrap(bus);
    ...
    it('covers DD/FD CB rotate/RES/SET with register target, pure register under DD, and DD fallback', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        const seq = [
            0xdd, 0xcb, 0x01, 0x10, // RL (IX+1),B
            0xdd, 0xcb, 0x02, 0x80, // RES 0,(IX+2),B
            0xfd, 0xcb, 0xff, 0xc7, // SET 0,(IY-1),A
            0xdd, 0x44, // LD B,H (pure reg under DD)
            0xdd, 0x39, // DD 39 fallback
        ];
        mem.set(seq, 0x0400);
        const rd = wrap(bus);
    ...
    it('covers base CALL/RET/RST and unknown base DB', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        const seq = [
            ...
            0xc9, // RET
            0xc7, // RST 00
            0x08, // EX AF,AF' (not in disasm mapping -> DB 08)
        ];
        mem.set(seq, 0x0500);
        const rd = wrap(bus);
        ...
        expect(disassembleOne(rd, 0x0505).text).toBe('DB 08');
    });
    it('covers IM 0 (ED 46), LD IX,nn and LD r,r\' plus disassembleRange()', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        const seq = [
            0xed, 0x46, // IM 0
            ...
            0x00, 0xfb, // NOP; EI for range test
        ];
        mem.set(seq, 0x0600);
        const rd = wrap(bus);
...
//# sourceMappingURL=z80_disasm_more.test.js.map