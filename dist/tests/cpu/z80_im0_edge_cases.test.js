...
describe('Z80 IM0 edge cases', () => {
    it('EI-delayed acceptance with multi-byte instruction (JP nn): IRQ accepted only after the next instruction completes', () => {
    ...
    it('Back-to-back IRQs: second IRQ not accepted until EI re-enables interrupts', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('Injected IM0 opcode persists across multiple interrupts (e.g., RST 08h twice)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Main @0000: IM 0; EI; NOP; HALT
        mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
    ...
    it('Injected opcode takes precedence over configured IM0 vector', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Main @0000: IM 0; EI; NOP; HALT
        mem.set([0xed, 0x46, 0xfb, 0x00, 0x76], 0x0000);
...
//# sourceMappingURL=z80_im0_edge_cases.test.js.map