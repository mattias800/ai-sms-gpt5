...
describe('Z80 onTrace hook', () => {
    it('reports instruction execution (NOP)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        mem[0x0000] = 0x00; // NOP
        const events = [];
        const cpu = createZ80({ bus, onTrace: (ev) => { events.push(ev); } });
    ...
    it('reports interrupt acceptance with null opcode (IM1 after EI+HALT)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Program: EI; HALT
...
//# sourceMappingURL=z80_trace_hook.test.js.map