...
describe('Z80 ED IN r,(C) and OUT (C),r', () => {
    it('IN B,(C) reads 0xFF from port 0x7f, sets flags, 12 cycles', () => {
    ...
    it('OUT (C),A takes 12 cycles and leaves flags unaffected', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('IN (C) (ED 70) updates flags from port and does not change registers', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // LD C,0x7f; IN (C)
    ...
    it('OUT (C),0 (ED 71) takes 12 cycles', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // LD C,0x7f; OUT (C),0
...
//# sourceMappingURL=z80_ed_in_out_c.test.js.map