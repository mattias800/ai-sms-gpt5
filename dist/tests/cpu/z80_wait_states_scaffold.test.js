...
describe('Z80 wait-state scaffolding (disabled by default)', () => {
    it('Default (disabled) does not affect cycles; hooks can report last wait cycles of previous step', () => {
    ...
    it('When enabled, memPenalty accumulates during a step and is exposed on the next step', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('LD (HL),n triggers penalties for opcode fetch + immediate read + memory write', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
...
//# sourceMappingURL=z80_wait_states_scaffold.test.js.map