...
describe('Z80 ADC/SBC A instructions', () => {
    it('ADC A,B without carry updates A, flags, cycles', () => {
    ...
    it('ADC A,n with carry produces signed overflow 0x7F + carry 1 -> 0x80', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ADC A,(HL) 7 cycles reads from memory', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // ADC A,(HL)
        mem.set([0x8e], 0x0020);
    ...
    it('SBC A,B with carry set computes 0 - 0 - 1 = 0xFF with flags', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('SBC A,n without carry: 0x80 - 0x01 = 0x7F, PV set, C clear, H set', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
...
//# sourceMappingURL=z80_adc_sbc.test.js.map