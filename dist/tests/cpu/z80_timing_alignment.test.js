...
describe('Z80 timing alignment (selected instructions)', () => {
    it('JP cc,nn is 10 cycles whether taken or not', () => {
    ...
    it('CALL cc,nn is 17 cycles if taken, 10 if not', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // CALL C,0x2000; HALT
    ...
    it('RET cc is 11 cycles if taken, 5 if not', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // RET NZ; HALT
    ...
    it('CB RLC r is 8 cycles; RLC (HL) is 15 cycles', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        const cpu = createZ80({ bus });
        // RLC B
    ...
    it('CB BIT 3,B is 8 cycles; BIT 3,(HL) is 12 cycles', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        const cpu = createZ80({ bus });
        // BIT 3,B => CB 58
    ...
    it('LD (nn),A and LD A,(nn) each take 13 cycles', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // LD (0x4000),A ; HALT
    ...
    it('INC r is 4 cycles; INC (HL) is 11 cycles', () => {
        const bus = new SimpleBus();
        ...
        // INC (HL)
        const st = cpu.getState();
        bus.getMemory()[0x0100] = 0x34; // value at (HL)
        bus.getMemory()[0x0001] = 0x76; // HALT after
        cpu.setState({ ...st, pc: 0x0101, h: 0x01, l: 0x00 });
        bus.getMemory()[0x0101] = 0x34; // placeholder; will be overwritten below
        bus.getMemory()[0x0101] = 0x34;
        bus.getMemory()[0x0101] = 0x34;
...
//# sourceMappingURL=z80_timing_alignment.test.js.map