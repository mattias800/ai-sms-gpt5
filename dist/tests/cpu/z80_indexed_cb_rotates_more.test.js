...
const setCarry = (cpu, c) => {
    const st = cpu.getState();
    cpu.setState({ ...st, f: c ? st.f | FLAG_C : st.f & ~FLAG_C });
};
describe('DD CB d rotates/shifts coverage on (IX+d)', () => {
    it('covers RL, RR(with carry), SLA, SRA, SLL, SRL and updates memory (23 cycles each)', () => {
...
//# sourceMappingURL=z80_indexed_cb_rotates_more.test.js.map