...
describe('Z80 ED-prefixed and block operation coverage', () => {
    it('ED 5A: ADC HL,DE updates HL and takes 15 cycles', () => {
    ...
    it('ED 73: LD (nn),SP stores SP at address, and ED 5B: LD DE,(nn) loads DE', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED B0: LDIR repeat path (21 cycles) then final (16 cycles)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED B1: CPIR repeat path then stop when match found', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 57: LD A,I sets PV when IFF2=1 and preserves C', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 5F: LD A,R clears PV when IFF2=0 and preserves C', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 4D: RETI pops PC and sets IFF1 := IFF2 (same as RETN)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 45: RETN pops PC and sets IFF1 := IFF2', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 47: LD I,A and ED 4F: LD R,A update special registers and take 9 cycles each', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ED 66: IM0 via alternate encoding and IRQ vector to default 0x0038', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
...
//# sourceMappingURL=z80_ed_block_coverage.test.js.map