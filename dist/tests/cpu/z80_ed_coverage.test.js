...
describe('Z80 ED coverage boosts', () => {
    it('unimplemented ED opcode throws', () => {
    ...
    it('LD A,I with iff2=false sets PV=0 and Z from I', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('IM 0 via ED 66 also sets IM mode', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('LD (nn),HL and LD BC,(nn) paths', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // LD (0x4008),HL
    ...
    it('LD A,R with iff2=false keeps PV=0', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
    ...
    it('ADC HL,DE and SBC HL,SP exercise ss selections', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // ADC HL,DE; HALT
    ...
    it('LD (nn),BC and LD DE,(nn) exercise both value paths', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // LD (0x4010),BC
...
//# sourceMappingURL=z80_ed_coverage.test.js.map