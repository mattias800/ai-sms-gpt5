...
const setRegs = (cpu, regs) => {
    const s = cpu.getState();
    cpu.setState({ ...s, ...regs });
};
describe('Z80 ED block transfer: LDI/LDD/LDIR/LDDR', () => {
    it('LDI copies byte, updates HL/DE++, BC--, flags and cycles (16)', () => {
    ...
    it('LDD copies byte, updates HL/DE--, BC--, flags and cycles (16)', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Program: LDD; HALT
    ...
    it('LDIR repeats until BC==0 with 21 cycles when repeating then 16 on last', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Program: LDIR; HALT
    ...
    it('LDDR repeats decrementing until BC==0 with expected cycles', () => {
        const bus = new SimpleBus();
        const mem = bus.getMemory();
        // Program: LDDR; HALT
...
//# sourceMappingURL=z80_ed_block.test.js.map