...
describe('SMS IO ports', () => {
    it('controller reads (0xDC/0xDD) return 0xFF by default', () => {
        const vdp = createVDP();
        const psg = createPSG();
    ...
    it('IO control (0x3F) and memory control (0x3E) writes are stored and not mirrored to VDP', () => {
        const vdp = createVDP();
        const psg = createPSG();
        const bus = new SmsBus(makeRom(), vdp, psg);
    ...
    it('VDP mirroring excludes 0x3E/0x3F and 0x7F', () => {
        const vdp = createVDP();
        const psg = createPSG();
        const bus = new SmsBus(makeRom(), vdp, psg);
...
//# sourceMappingURL=sms_io.test.js.map