...
describe('Conservative I/O control scaffolding (0x3F) for controller reads', () => {
    it('defaults to 0xFF on 0xDC/0xDD and remains 0xFF when outputs default high', () => {
        const vdp = createVDP();
        const psg = createPSG();
        ...
        bus.__setIOMaskForTest(0x3f, 0xff);
        expect(bus.readIO8(0xdc)).toBe(0xff);
        expect(bus.readIO8(0xdd)).toBe(0xff);
    });
    it('when masking drives some lines low, 0xDC/0xDD reflect driven bits and unaffected bits remain pulled-up', () => {
        const vdp = createVDP();
        const psg = createPSG();
        const bus = new SmsBus(makeRom(), vdp, psg);
        // Drive lower 2 bits as outputs low
        bus.__setIOMaskForTest(0x03, 0x00);
...
//# sourceMappingURL=io_control_mask.test.js.map