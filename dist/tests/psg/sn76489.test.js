...
const tone = (hi, lo) => ((hi & 0x3f) << 4) | (lo & 0x0f);
describe('SN76489 PSG basics', () => {
    it('latch+data and data-only update tone0 low/high correctly', () => {
        const psg = createPSG();
    ...
    it('updates tone1 and tone2 low/high correctly as well', () => {
        const psg = createPSG();
    ...
    it('updates noise control and volume (noise channel) via latch writes', () => {
        const psg = createPSG();
    ...
    it('tickCycles is a no-op and getState returns clones', () => {
        const psg = createPSG();
        psg.write(0x80 | 0x01);
        psg.write(0x90 | 0x00);
        psg.write(0x3f);
...
//# sourceMappingURL=sn76489.test.js.map