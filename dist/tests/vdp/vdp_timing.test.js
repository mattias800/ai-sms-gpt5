...
describe('VDP timing (HCounter/VCounter/VBlank)', () => {
    it('HCounter front porch returns 0x00, end-of-line returns 0xB0', () => {
        const vdp = createVDP();
    ...
    it('VCounter increases after a full line and VBlank sets status bit 7', () => {
        const vdp = createVDP();
        const v0 = vdp.readPort(0x7f) & 0xff;
        const gs = vdp.getState?.();
        expect(gs).toBeTruthy();
        const cpl = gs.cyclesPerLine;
        ...
        const statusBefore = vdp.getState?.().status;
        ...
        const statusAfter = vdp.getState?.().status;
        expect((statusAfter & 0x80) !== 0).toBe(false);
    });
});
//# sourceMappingURL=vdp_timing.test.js.map