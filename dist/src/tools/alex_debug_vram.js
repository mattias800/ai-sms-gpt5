...
for (let i = 0; i < 32; i++) {
    ...
    if (val !== 0) {
        ...
        const g = ((val >> 2) & 3);
        const b = (val & 3);
        console.log(`CRAM[${i.toString(16).padStart(2, '0')}] = 0x${val.toString(16).padStart(2, '0')} (R=${r} G=${g} B=${b})`);
    }
}
if (!hasColor) {
...
for (let i = 0; i < 32 * 24 * 2 && nameCount < 10; i += 2) {
    ...
    if (entry !== 0) {
        ...
        console.log(`  Name[${(i / 2).toString(16)}] @ VRAM[0x${addr.toString(16)}] = 0x${entry.toString(16).padStart(4, '0')} (tile ${tileIndex})`);
        nameCount++;
    }
}
// Check pattern data
const patternBase = ((vdpState.regs[4] & 0x07) << 11);
...
for (let tile = 0; tile < 512 && tileCount < 5; tile++) {
    ...
    if (hasData) {
        ...
        for (let row = 0; row < 8; row++) {
            const b0 = vdpState.vram[addr + row * 4] ?? 0;
            const b1 = vdpState.vram[addr + row * 4 + 1] ?? 0;
            const b2 = vdpState.vram[addr + row * 4 + 2] ?? 0;
            const b3 = vdpState.vram[addr + row * 4 + 3] ?? 0;
            let pixels = '';
...
console.log('\n=== Scrolling ===');
console.log(`Horizontal scroll (R8): ${vdpState.regs[8]} (shifts screen ${256 - vdpState.regs[8]} pixels left)`);
console.log(`Vertical scroll (R9): ${vdpState.regs[9]} (shifts screen ${vdpState.regs[9]} pixels up)`);
// Check sprite attribute table
const spriteTableBase = ((vdpState.regs[5] & 0x7E) << 7);
console.log(`\n=== Sprite Attribute Table (base=0x${spriteTableBase.toString(16)}) ===`);
let spriteCount = 0;
for (let i = 0; i < 64; i++) {
    const addr = (spriteTableBase + i) & 0x3fff;
    const y = vdpState.vram[addr] ?? 0;
    if (y === 0xD0)
        break; // End marker
    if (y > 0 && y < 0xE0) {
        const x = vdpState.vram[(spriteTableBase + 128 + i * 2) & 0x3fff] ?? 0;
        const pattern = vdpState.vram[(spriteTableBase + 128 + i * 2 + 1) & 0x3fff] ?? 0;
