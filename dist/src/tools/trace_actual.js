        ...
        trace: {
            onTrace: (ev) => {
                ...
                if (pc === 0x0284 && atJump) {
                    ...
                    console.log('Memory at 0x0284:', bytes.map(b => b.toString(16).padStart(2, '0')).join(' '));
                    // Try to disassemble
                    const readFn = (addr) => bytes[addr - 0x0284] ?? 0;
                ...
                if (step > 50 && atJump) {
                    ...
                    if (lastFew.size < 5) {
                        console.log('\nStuck in a loop with PCs:', Array.from(lastFew).map(p => `0x${p.toString(16).padStart(4, '0')}`).join(', '));
                    }
                    process.exit(0);
                }
                step++;
...
//# sourceMappingURL=trace_actual.js.map