...
function vdpSetReg(bus, reg, val) {
    bus.writeIO8(0xbf, val & 0xff);
    bus.writeIO8(0xbf, 0x80 | (reg & 0x0f));
}
function vdpSetAddrCode(bus, addr, code) {
    bus.writeIO8(0xbf, addr & 0xff);
    bus.writeIO8(0xbf, ((addr >>> 8) & 0x3f) | ((code & 0x03) << 6));
}
function vdpWriteData(bus, data) {
    for (const v of data)
        bus.writeIO8(0xbe, v & 0xff);
}
function makeTilePlanes(rows) {
...
function forceMinimalFrame(bus) {
...
async function main() {
    ...
    bus1.readIO8 = (port) => {
        ...
        if (!noHC && p === 0x7e) {
            ...
            if (hcReadCount === 1)
                return 0x00; // Initial read
            if (hcReadCount === 2)
                return 0xb0; // Pass first wait loop
            if (hcReadCount < 20) {
        ...
        if (p === 0x7f) {
            ...
            if (line >= vblankStart)
                return (0xc0 + (line - vblankStart)) & 0xff;
            return line & 0xff;
        }
        return __origReadIO8(p);
    };
    ...
    nonZeroVRAM = 0;
    for (let i = 0; i < vramU8b.length; i++)
        if (vramU8b[i] !== 0)
            nonZeroVRAM++;
    const preDiag = {
    ...
    if (nonZeroVRAM < 512) {
        ...
        nonZeroVRAM = 0;
        for (let i = 0; i < vramU8c.length; i++)
            if (vramU8c[i] !== 0)
                nonZeroVRAM++;
    }
    ...
    const diagOut = { ...diag, lastPC: `0x${lastPC.toString(16)}` };
...
//# sourceMappingURL=headless_sonic.js.map