...
const buildRom = (): Uint8Array => {
  ...
  const outBF = (): void => { emit(0xD3); emit(0xBF); };

  // Enable VBlank IRQ (reg1 bit5)
  emit(0x3E); emit(0x20); // LD A,0x20
  outBF();                // OUT (BF),A (low)
  emit(0x3E); emit(0x81); // LD A,0x81 (hi: 0x80|1)
  ...
  a = 0x0038;
  const outBE = (): void => { emit(0xD3); emit(0xBE); };

  // Set autoincrement reg15=1
  emit(0x3E); emit(0x01); // LD A,0x01
  outBF();                // OUT (BF),A (low)
  emit(0x3E); emit(0x8F); // LD A,0x8F (hi: 0x80|15)
  outBF();                // OUT (BF),A (reg 15)

  // Set CRAM address to 0x0000 (code=3)
  emit(0x3E); emit(0x00); // LD A,0x00
  outBF();                // OUT (BF),A (lo)
  emit(0x3E); emit(0xC0); // LD A,0xC0 (hi with code=3)
  outBF();

  // Write 2 CRAM bytes: 0x2A, 0x3F
  emit(0x3E); emit(0x2A); // LD A,0x2A
  outBE();
  emit(0x3E); emit(0x3F); // LD A,0x3F
  outBE();

  // Enable display: reg1 bit6 plus keep bit5 => 0x60
  emit(0x3E); emit(0x60); // LD A,0x60
  outBF();
  emit(0x3E); emit(0x81); // LD A,0x81
  outBF();
