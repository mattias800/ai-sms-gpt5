...
describe('Z80 additional DEC dd branches and POP qq variants', (): void => {
  ...
  it('POP BC/DE/HL read from stack and update SP', (): void => {
    ...
    mem[0x9002] = 0x78; mem[0x9003] = 0x56;
    mem[0x9004] = 0xbc; mem[0x9005] = 0x9a;

    let c = step(cpu); expect(c).toBe(10); // POP BC
    st = cpu.getState();
    expect(st.b).toBe(0x12); expect(st.c).toBe(0x34); expect(st.sp).toBe(0x9002);

    c = step(cpu); expect(c).toBe(10); // POP DE
    st = cpu.getState();
    expect(st.d).toBe(0x56); expect(st.e).toBe(0x78); expect(st.sp).toBe(0x9004);

    c = step(cpu); expect(c).toBe(10); // POP HL
    st = cpu.getState();
    expect(st.h).toBe(0x9a); expect(st.l).toBe(0xbc); expect(st.sp).toBe(0x9006);
  });
});
