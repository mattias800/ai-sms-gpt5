...
import { createZ80, IZ80 } from '../../src/cpu/z80/z80.js';
import { FLAG_C, FLAG_H, FLAG_N, FLAG_PV, FLAG_S } from '../../src/cpu/z80/flags.js';

const runSteps = (cpu: IZ80, steps: number): number => {
  let cycles = 0;
  for (let i = 0; i < steps; i++) cycles += cpu.stepOne().cycles;
  return cycles;
};

describe('Z80 basic ops', (): void => {
  it('LD r,n; LD r,r; NOP; HALT', (): void => {
  ...
  it('INC/DEC r flags and C preservation', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('ADD/SUB/AND/XOR/OR/CP with immediate and registers', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program:
