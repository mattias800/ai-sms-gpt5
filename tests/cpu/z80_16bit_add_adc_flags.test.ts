...
describe('Z80 16-bit ADD/ADC HL flags (H and C paths)', (): void => {
  it('ADD HL,BC sets H without C for 0x0FFF + 0x0001', (): void => {
  ...
  it('ADD HL,SP sets both H and C for 0xFFFF + 0x0001', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('ED: ADC HL,DE with carry in', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
