...
describe('Z80 conditional coverage and ED SBC HL,ss', (): void => {
  it('covers JP cc for NC/C/PO/PE/P/M cases via flags', (): void => {
    interface Case {
      op: number;
      setF: (f0: number) => number;
      expectJump: boolean;
    }
    const cases: Case[] = [
      // JP NC,nn (0xD2): take when C==0 -> make not taken by setting C=1
      { op: 0xd2, setF: (f: number): number => f | 0x01, expectJump: false },
      // JP C,nn (0xDA): take when C==1
      { op: 0xda, setF: (f: number): number => f | 0x01, expectJump: true },
      // JP PO,nn (0xE2): take when PV==0
      { op: 0xe2, setF: (f: number): number => f & ~0x04, expectJump: true },
      // JP PE,nn (0xEA): take when PV==1
      { op: 0xea, setF: (f: number): number => f | 0x04, expectJump: true },
      // JP P,nn (0xF2): take when S==0
      { op: 0xf2, setF: (f: number): number => f & ~0x80, expectJump: true },
      // JP M,nn (0xFA): take when S==1
      { op: 0xfa, setF: (f: number): number => f | 0x80, expectJump: true },
    ];
  ...
  it('CALL cc taken path (CALL Z,nn)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // CALL Z,0040h
  ...
  it('RET cc taken path (RET C)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('ED 42: SBC HL,BC updates HL and takes 15 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // SBC HL,BC
