...
describe('Z80 R refresh register semantics', (): void => {
  it('R increments by 1 on a simple opcode fetch (NOP)', (): void => {
  ...
  it('LD A,R (ED 5F) causes two M1 increments (prefix + opcode)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // ED 5F ; HALT
  ...
  it('DD CB d op sequence increments R three times (DD + CB + op)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
