...
describe('Z80 DD/FD indexed addressing (IX/IY)', (): void => {
  it('LD (IX+d),n stores byte and takes 19 cycles', (): void => {
  ...
  it('LD A,(IX+d) and LD (IX+d),A roundtrip (19 cycles each)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('INC (IX+d) and DEC (IX+d) update flags and take 23 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('ADD A,(IX+d) and SUB (IY+d) with cycles and flags', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('LD B,(IY+d) and LD (IY+d),B operate like IX variants', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('supports DD/FD register variants: LD B,IXH via DD 44', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
