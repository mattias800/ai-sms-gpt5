...
describe('Additional branch coverage for Z80', (): void => {
  it('IM2 IRQ accepted immediately (preempts NOP) when next op is not HALT', (): void => {
  ...
  it('FD-prefixed IY instructions (LD IY,nn and INC IY)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
