...
describe('DD/FD CB d operations on (IX/IY+d)', (): void => {
  it('DD CB d RLC (IX+d),B writes back and transfers to B (23 cycles)', (): void => {
  ...
  it('DD CB d BIT 7,(IX+d) sets flags and takes 20 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('FD CB d RES and SET operate on memory and optional register target (23 cycles)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
