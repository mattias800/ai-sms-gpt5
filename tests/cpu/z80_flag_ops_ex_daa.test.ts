...
describe('Z80 accumulator rotates/flag ops, EX/EXX, DAA', (): void => {
  it('RLCA and RRCA update A and C correctly', (): void => {
  ...
  it('RLA and RRA rotate through carry', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('CPL sets H and N, preserves C', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('SCF sets carry and clears H/N; CCF toggles carry and sets H=prevC', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('DAA adjusts A correctly after addition (0x9A + adjust = 0x00 with C and Z set)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // DAA
    mem.set([0x27], 0x0000);
  ...
  it("EX AF,AF' swaps primary and alternate AF; EXX swaps BC/DE/HL with alternates", (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // EX AF,AF'; EXX
