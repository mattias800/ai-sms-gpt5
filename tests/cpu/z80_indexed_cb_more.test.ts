...
describe('DD/FD CB d additional coverage', (): void => {
  it('DD CB d RRC (IX+d),A updates memory and A; cycles 23', (): void => {
  ...
  it('DD CB d BIT 0,(IX+d) sets Z=1 and preserves C; cycles 20', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('FD CB d RES 7,(IY+d),A and SET 7,(IY+d),B; cycles 23', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
...
describe('DD/FD arithmetic with (IX/IY+d)', (): void => {
  it('AND/XOR/OR/CP A,(IY+d) all take 19 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Sequence: LD A,0xF0; AND (IY+0); LD A,0x55; XOR (IY+0); LD A,0x0F; OR (IY+0); LD A,0x80; CP (IY+0); HALT
    mem.set(
      [
        0x3e, 0xf0, 0xfd, 0xa6, 0x00, 0x3e, 0x55, 0xfd, 0xae, 0x00, 0x3e, 0x0f, 0xfd, 0xb6, 0x00, 0x3e, 0x80, 0xfd,
        0xbe, 0x00, 0x76,
      ],
      0x0000
    );
