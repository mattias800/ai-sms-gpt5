...
describe('Z80 ED-prefixed instructions', (): void => {
  it('ADC HL,BC sets flags and takes 15 cycles', (): void => {
  ...
  it('SBC HL,DE with carry set', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('LD (nn),DE and LD HL,(nn) roundtrip, each 20 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('NEG sets correct flags and 8 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('IM 1 sets interrupt mode, 8 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('LD I,A and LD A,I; LD R,A and LD A,R with flags behavior', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // Program: LD A,0x55; LD I,A; LD A,0x00; LD A,I; LD A,0xAA; LD R,A; LD A,0x00; LD A,R; HALT
    mem.set(
      [0x3e, 0x55, 0xed, 0x47, 0x3e, 0x00, 0xed, 0x57, 0x3e, 0xaa, 0xed, 0x4f, 0x3e, 0x00, 0xed, 0x5f, 0x76],
      0x0000
    );
    const cpu = createZ80({ bus });
