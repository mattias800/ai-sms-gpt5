...
describe('Z80 CB and DD/FD CB coverage', (): void => {
  it('CB SRL C (register) updates C and flags, 8 cycles', (): void => {
  ...
  it('CB SLL (HL) modifies memory and sets carry, 15 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('CB BIT 7,(HL) sets S if bit set, preserves C and sets H, 12 cycles for (HL)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('CB BIT 2,(HL) clears S, sets Z/PV when bit clear, 12 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
  ...
  it('CB RES 3,(HL) and CB SET 5,B', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // RES 3,(HL)
  ...
  it('DD CB BIT 6,(IX+1) 20 cycles; DD CB RES 1,(IX+2),E and RL (IX+1),D 23 cycles', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // BIT 6,(IX+1)
  ...
  it('CB RLC B parity true (result 0x00) and CB RRC C parity false (result 0x80)', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // RLC B
  ...
  it('CB SLA D sets carry and clears PV for odd parity; CB SRA A preserves sign bit and sets PV for even parity', (): void => {
    const bus = new SimpleBus();
    const mem = bus.getMemory();
    // SLA D
