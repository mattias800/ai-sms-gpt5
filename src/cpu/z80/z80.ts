...
import { Z80State, createResetState } from './state.js';
...
export interface IZ80 {
  ...
  requestIRQ: () => void;
  requestNMI: () => void;
  setIM2Vector: (v: number) => void;
  setIM0Vector: (addr: number) => void;
  setIM0Opcode: (op: number | null) => void;
  resetInterruptConfig: () => void;
  // Wait-state scaffolding
  setWaitStateHooks: (cfg: WaitStateHooks | null) => void;
  getLastWaitCycles: () => number;
}

export interface WaitStateHooks {
  enabled?: boolean | undefined; // default false
  memPenalty?: (addr: number, isWrite: boolean) => number | undefined; // extra cycles per memory access
  ioPenalty?: (port: number, isWrite: boolean) => number | undefined; // extra cycles per IO access
  includeWaitInCycles?: boolean | undefined; // when true, reported cycles include accumulated wait penalties
  // Legacy compatibility (prior hooks API)
  onMemoryRead?: (addr: number) => number | undefined;
  onMemoryWrite?: (addr: number) => number | undefined;
  onIORead?: (port: number) => number | undefined;
  onIOWrite?: (port: number) => number | undefined;
}
...
export interface TraceEvent {
  pcBefore: number;
  opcode: number | null; // null when no opcode fetched (e.g., interrupt acceptance while halted or before fetch)
  cycles: number; // cycles returned by step (includes waits if configured)
  irqAccepted: boolean;
  nmiAccepted: boolean;
...
export interface CreateZ80Options {
  ...
  traceDisasm?: boolean; // when true, include disassembled text/bytes in trace events
  traceRegs?: boolean; // when true, include register snapshot in trace events
...
export const createZ80 = (opts: CreateZ80Options): IZ80 => {
  let s: Z80State = createResetState();
  const bus = opts.bus;

  // Wait-state scaffolding (disabled by default)
  let ws: WaitStateHooks | null = opts.waitStates ?? null;
  ...
  const wsEnabled = (): boolean => !!(ws && ws.enabled);
  const addMemPenalty = (addr: number, isWrite: boolean): void => {
    if (!wsEnabled()) return;
    let p = 0;
    if (ws?.memPenalty) p = ws.memPenalty(addr & 0xffff, isWrite) ?? 0;
    else if (!isWrite && ws?.onMemoryRead) p = ws.onMemoryRead(addr & 0xffff) ?? 0;
    else if (isWrite && ws?.onMemoryWrite) p = ws.onMemoryWrite(addr & 0xffff) ?? 0;
    if (p && p > 0) curWaitCycles += p | 0;
  };
  const addIoPenalty = (port: number, isWrite: boolean): void => {
    if (!wsEnabled()) return;
    let p = 0;
    if (ws?.ioPenalty) p = ws.ioPenalty(port & 0xff, isWrite) ?? 0;
    else if (!isWrite && ws?.onIORead) p = ws.onIORead(port & 0xff) ?? 0;
    else if (isWrite && ws?.onIOWrite) p = ws.onIOWrite(port & 0xff) ?? 0;
    if (p && p > 0) curWaitCycles += p | 0;
  };
...
const readIO8 = (port: number): number => {
  ...
  const add8 = (a: number, b: number, carry: 0 | 1): { r: number; f: number } => {
    ...
    let f = setSZ53(res);
    if (hc) f |= FLAG_H;
    if (overflow) f |= FLAG_PV;
    if (c) f |= FLAG_C;
    // N cleared
    return { r: res, f };
  };

  const sub8 = (a: number, b: number, carry: 0 | 1): { r: number; f: number } => {
    const res = (a - b - carry) & 0xff;
  ...
  const inc8 = (x: number, fPrev: number): { r: number; f: number } => {
    const r = (x + 1) & 0xff;
    ...
    if (x === 0x7f) f |= FLAG_PV;
    // N reset; preserve C
    if (fPrev & FLAG_C) f |= FLAG_C;
    return { r, f };
  };

  const dec8 = (x: number, fPrev: number): { r: number; f: number } => {
    const r = (x - 1) & 0xff;
    const hb = (x & 0x0f) - 1 < 0;
  ...
  const logicFlags = (r: number, h: 0 | 1): number => {
    let f = setSZ53(r);
    if (h) f |= FLAG_H;
    if (parity8(r)) f |= FLAG_PV;
    // N=0, C=0
    return f;
  };

  const regGet = (code: number): number => {
    switch (code & 7) {
      case 0:
        return s.b;
      case 1:
  ...
  const regSet = (code: number, val: number): void => {
    const v = val & 0xff;
    switch (code & 7) {
      case 0:
        s.b = v;
  ...
  const write16 = (addr: number, val: number): void => {
    write8(addr, val & 0xff);
    write8((addr + 1) & 0xffff, (val >>> 8) & 0xff);
  };

  const s8 = (n: number): number => ((n & 0x80) !== 0 ? n - 0x100 : n);

  const cond = (cc: number): boolean => {
  ...
  const stepOne = (): StepResult => {
    ...
    const mkRes = (baseCycles: number, irqA: boolean, nmiA: boolean): StepResult => {
      ...
      if (tracer) {
        ...
        if (traceWithDisasm && currOp !== null) {
          ...
          const r = disassembleOne((addr: number): number => bus.read8(addr & 0xffff) & 0xff, pc0);
          text = r.text;
          bytes = [...r.bytes];
        }
      ...
      if (pendingIRQ && s.iff1 && !blockIRQThisStep) {
        ...
        if (s.im === 2) {
          ...
          if (im0Opcode !== null) {
            ...
            const rstTargets: Record<number, number> = {
              0xc7: 0x00,
              0xcf: 0x08,
              0xd7: 0x10,
              0xdf: 0x18,
    ...
    if (op === 0xcb) {
      ...
      const doWrite = (val: number): void => {
        if (rCode === 6) write8(getHL(), val & 0xff);
        else regSet(rCode, val & 0xff);
      };
      const doRead = (): number => (rCode === 6 ? read8(getHL()) : regGet(rCode));
      ...
      if (group === 0x40) {
        ...
        let f = 0;
        if (!bitSet) f |= FLAG_Z | FLAG_PV; // Z and PV follow test result
        if (y === 7 && bitSet) f |= FLAG_S;
        f |= FLAG_H; // H set
        // Preserve C
    ...
    if (op === 0xed) {
      ...
      if ((sub & 0xcf) === 0x4a) {
        ...
        if (r16 === 0) f |= FLAG_Z;
        if ((((a ^ ss ^ r16) >>> 12) & 1) !== 0) f |= FLAG_H; // half carry from bit 11
        if (~(a ^ ss) & (a ^ r16) & 0x8000) f |= FLAG_PV;
      ...
      if ((sub & 0xcf) === 0x42) {
        ...
        if (r16 & 0x8000) f |= FLAG_S;
        if (r16 === 0) f |= FLAG_Z;
        if ((((a ^ ss ^ r16) >>> 12) & 1) !== 0) f |= FLAG_H; // half borrow
        if ((a ^ ss) & (a ^ r16) & 0x8000) f |= FLAG_PV;
        if (a < ss + carry) f |= FLAG_C;
        const hi = (r16 >>> 8) & 0xff;
        if (hi & 0x20) f |= FLAG_5;
        if (hi & 0x08) f |= FLAG_3;
        s.h = (r16 >>> 8) & 0xff;
      ...
      if (sub === 0xa1 || sub === 0xa9 || sub === 0xb1 || sub === 0xb9) {
        ...
        if (adj & 0x08) f |= FLAG_3;
        if (adj & 0x20) f |= FLAG_5;
        s.f = f;
    ...
    if (op === 0xdd || op === 0xfd) {
      const isIX = op === 0xdd;
      const indexVal = (): number => (isIX ? s.ix : s.iy) & 0xffff;
      const op2 = fetchOpcode();

      // Helpers for DD/FD 8-bit register mapping (H/L -> IXH/IYH and IXL/IYL)
      const xyh = (): number => ((isIX ? s.ix : s.iy) >>> 8) & 0xff;
      const xyl = (): number => (isIX ? s.ix : s.iy) & 0xff;
      ...
      if ((op2 & 0xcf) === 0x09) {
        ...
        if ((((a ^ ppVal ^ r16) >>> 12) & 1) !== 0) f |= FLAG_H;
        if (sum > 0xffff) f |= FLAG_C;
        const hi = (r16 >>> 8) & 0xff;
        if (hi & 0x20) f |= FLAG_5;
        if (hi & 0x08) f |= FLAG_3;
      ...
      if (op2 === 0xcb) {
        ...
        if (group === 0x80 || group === 0xc0) {
          let newV = v;
          if (group === 0x80) newV = v & (~(1 << y) & 0xff);
          else newV = v | (1 << y);
          write8(addr, newV);
          if (rCode !== 6) regSet(rCode, newV);
          return mkRes(23, false, false);
        }
      ...
      if (
        ...
        if (arithSrc === 6) {
          ...
          if ((op2 & 0xf8) === 0x80) {
            ...
            const carry = (s.f & FLAG_C) !== 0 ? (1 as 0 | 1) : (0 as 0 | 1);
    ...
    if ((op & 0xcf) === 0x09) {
      ...
      let f = s.f & (FLAG_S | FLAG_Z | FLAG_PV);
      if ((((hl ^ ssVal ^ r16) >>> 12) & 1) !== 0) f |= FLAG_H;
      if (sum > 0xffff) f |= FLAG_C;
      const hi = (r16 >>> 8) & 0xff;
      if (hi & 0x20) f |= FLAG_5;
      if (hi & 0x08) f |= FLAG_3;
    ...
    if ((op & 0xcf) === 0xc5) {
      const qq = (op >>> 4) & 3;
      let val = 0;
      if (qq === 0) val = ((s.b << 8) | s.c) & 0xffff;
      else if (qq === 1) val = ((s.d << 8) | s.e) & 0xffff;
      else if (qq === 2) val = ((s.h << 8) | s.l) & 0xffff;
      else val = ((s.a << 8) | s.f) & 0xffff;
      push16(val);
      return mkRes(11, false, false);
    }
    if ((op & 0xcf) === 0xc1) {
    ...
    if (op === 0x3f) {
      ...
      let f = s.f & (FLAG_S | FLAG_Z | FLAG_PV);
      if (cPrev === 0) f |= FLAG_C;
      if (cPrev) f |= FLAG_H; // H becomes previous C
      if (s.a & 0x20) f |= FLAG_5;
      if (s.a & 0x08) f |= FLAG_3;
      s.f = f; // N=0
      return mkRes(4, false, false);
    ...
    if (op === 0x27) {
      ...
      if ((f & FLAG_N) === 0) {
        if ((f & FLAG_H) !== 0 || (a & 0x0f) > 9) adjust += 0x06;
        if (carry || a > 0x99) {
          adjust += 0x60;
          carry = true;
        }
        ...
        if ((f & FLAG_H) !== 0) adjust += 0x06;
        if (carry) adjust += 0x60;
        a = (a - adjust) & 0xff;
      }
      s.a = a;
      let nf = setSZ53(a);
      if (parity8(a)) nf |= FLAG_PV;
      if (carry) nf |= FLAG_C;
      if (f & FLAG_N) nf |= FLAG_N;
      s.f = nf;
      return mkRes(4, false, false);
    }
  ...
  const getState = (): Z80State => ({ ...s });
  const setState = (st: Z80State): void => {
    s = { ...st };
  };
  ...
  const setIM2Vector = (v: number): void => {
    im2Vector = v & 0xff;
  };

  const setIM0Vector = (addr: number): void => {
    im0Vector = addr & 0xffff;
  };

  const setIM0Opcode = (op: number | null): void => {
    if (op === null) im0Opcode = null;
    else im0Opcode = op & 0xff;
  };

  const resetInterruptConfig = (): void => {
    im0Opcode = null;
    im0Vector = 0x0038;
    im2Vector = 0xff;
  };
  ...
  return {
    ...
    getLastWaitCycles: (): number => lastWaitCycles | 0,
  };
};
