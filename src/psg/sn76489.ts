...
export const createPSG = (): IPSG => {
  ...
  const getSample = (): number => {
    ...
    for (let ch = 0; ch < 3; ch++) {
      const vol = state.vols[ch] & 0x0f;
      const amp = volumeTable[vol] | 0;
      if (amp > 0) {
        mixed += state.outputs[ch] ? amp : -amp;
        hasSound = true;
      }
    }

    // Mix noise channel as signed
    const noiseVol = state.vols[3] & 0x0f;
    const noiseAmp = volumeTable[noiseVol] | 0;
    if (noiseAmp > 0) {
    ...
    if (hasSound && soundDebugCounter++ % 44100 === 0) {
