...
console.log('\nVDP Registers:');
console.log('R2 (Name table):', vdpState.regs[2].toString(16), '-> Base:', ((vdpState.regs[2] & 0x0e) << 10).toString(16));
console.log('R4 (Pattern):', vdpState.regs[4].toString(16), '-> Bit 2:', (vdpState.regs[4] & 0x04) !== 0);
...
console.log('Unique non-zero tiles:', Array.from(uniqueTiles).slice(0, 10).join(', '), uniqueTiles.size > 10 ? '...' : '');
...
if (sampleTile !== undefined) {
  ...
  console.log('At base 0x0000 (addr', patternAddr0.toString(16) + '):', 
    vdpState.vram[patternAddr0] !== 0 ? 'Has data' : 'Empty');
  console.log('At base 0x2000 (addr', patternAddr2000.toString(16) + '):', 
    vdpState.vram[patternAddr2000] !== 0 ? 'Has data' : 'Empty');
}
