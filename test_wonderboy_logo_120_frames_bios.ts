...
// Minimal PNG encoder (same as in non-BIOS test)
...
const main = (): number => {
  console.log('Headless Wonder Boy (BIOS): run 120 frames and verify SEGA logo/light blue background\n');
  const romData = readFileSync('wonderboy5.sms');
  // Prefer explicit env override, then project/local paths
  let biosData: Uint8Array;
  const envPath = process.env.SMS_BIOS_FILE;
  if (envPath) {
    biosData = new Uint8Array(readFileSync(envPath));
    console.log(`Using BIOS (env): ${envPath}`);
  } else {
    try {
  ...
  if (!vdp.renderFrame) { console.error('renderFrame not available'); return 2; }
  const frame = vdp.renderFrame();
  if (!frame || frame.length !== 256*192*3) { console.error('Invalid frame'); return 2; }

  const counts = new Map<string, number>();
  for (let y=0; y<192; y++) for (let x=0; x<256; x++) { const [r,g,b]=rgbAt(frame,x,y); const k=keyOf(r,g,b); counts.set(k,(counts.get(k)??0)+1); }
  let bgKey=''; let bgCount=0; for (const [k,c] of counts.entries()) { if (c>bgCount){ bgCount=c; bgKey=k; } }
  const bg = parseKey(bgKey); const bgShare = bgCount / (256*192);
  console.log(`Most common color: rgb(${bg[0]},${bg[1]},${bg[2]}) with ${(bgShare*100).toFixed(2)}% coverage`);

  if (!isLightBlueish(bg[0],bg[1],bg[2])) { console.error(`FAIL: Dominant background not light blue-ish: rgb(${bg[0]},${bg[1]},${bg[2]})`); dumpFrame(frame,'wonderboy_120_bios.png'); return 1; }
  if (bgShare < 0.60) { console.error(`FAIL: Background coverage too low: ${(bgShare*100).toFixed(2)}% (<60%)`); dumpFrame(frame,'wonderboy_120_bios.png'); return 1; }

  const cx=128, cy=96; const startX=cx-5, startY=cy-5; let nonBgCenter=0, sawWhite=0, sawDarkBlue=0; const unexpected = new Set<string>();
  for (let y=startY;y<startY+10;y++) for (let x=startX;x<startX+10;x++) { const [r,g,b]=rgbAt(frame,x,y); const k=keyOf(r,g,b); if (k===bgKey) continue; nonBgCenter++; if (isWhite(r,g,b)) { sawWhite++; continue; } if (isDarkerBlueThan(bg,[r,g,b])) { sawDarkBlue++; continue; } unexpected.add(k); }

  if (nonBgCenter===0) { console.error('FAIL: Center 10x10 contains only background'); dumpFrame(frame,'wonderboy_120_bios.png'); return 1; }
  if (unexpected.size>0) { console.error(`FAIL: Center 10x10 has unexpected colors: ${Array.from(unexpected).join(' | ')}`); dumpFrame(frame,'wonderboy_120_bios.png'); return 1; }
  if (sawWhite===0 || sawDarkBlue===0) { console.error(`FAIL: Center lacks both white and darker blue (white=${sawWhite}, darkBlue=${sawDarkBlue})`); dumpFrame(frame,'wonderboy_120_bios.png'); return 1; }

  console.log('PASS (BIOS): Background light blue; center contains white + darker blue (SEGA logo).');
  dumpFrame(frame,'wonderboy_120_bios.png');
  return 0;
};
