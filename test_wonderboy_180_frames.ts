...
const main = (): number => {
  console.log('Wonder Boy 180 frames test\n');
  ...
  if (!isLightBlueish(bg[0],bg[1],bg[2])) { 
    ...
    dumpFrame(frame,'wonderboy_180_fail.png'); 
    return 1; 
  }
  if (bgShare < 0.60) { 
  ...
  console.log('🎉 Wonder Boy graphics are working correctly after 180 frames!');
  dumpFrame(frame,'wonderboy_180_frames.png');
  return 0;
};
